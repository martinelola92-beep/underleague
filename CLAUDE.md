...
La fuente de verdad del diseño es `docs/requisitos.md` (v0.10). Cada requisito tiene identificador (`RF-xxx` funcional, `RT-xxx` técnico, `RA-xxx` arte, `UI-xxx` interfaz): cítalos en commits, ADRs, tests y comentarios cuando implementes o discutas uno.
...
- **Leer sin gastar contexto**: la salida de tests y los `summary.csv` se leen con `tools/test-resumen.sh` y `tools/balance-resumen.py` (skill `build-and-test`); "qué dice la documentación sobre X" se pregunta con `tools/opencode-consulta.sh`, que verifica cada cita contra el fichero (skill `opencode-worker`), antes que abrir documentos enteros o lanzar `Explore`.
