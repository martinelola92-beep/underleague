...
- `limit.per` (límite por): `play` (jugada), `match` (partido), `mob` (turba), `run`. (El ejemplo original decía `parte`; no existen partes, RF-055.) El uso se consume **al activarse**, antes de aplicar los efectos, igual que un consumible: un efecto que vuelve a publicar su propio disparador (`extraAction`) ya no encadena el perk más allá de su límite (BC-B, 19 sep 2026). Nota: un `per: play` que cierre la jugada dentro de sus propios efectos (un tiro) recupera el uso en ese mismo tick; hoy ningún perk del catálogo usa `per: play`.
