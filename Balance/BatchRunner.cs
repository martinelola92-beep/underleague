    ...
    public static BatchResult Run(Options options, Catalog catalog, ReferenceConfig reference)
    {
        ...
        for (int i = 0; i < plan.Length; i++)
        {
            ...
            matches.Add(new MatchRow(
                ...
                PassesAttempted: report.Players.Sum(p => p.PassesAttempted),
                PassesCompleted: report.Players.Sum(p => p.PassesCompleted),
                PassesIntercepted: report.PassesIntercepted[0] + report.PassesIntercepted[1],
                PassesLoose: report.PassesLoose[0] + report.PassesLoose[1],
                PassesBeaten: report.PassesBeaten[0] + report.PassesBeaten[1],
                ThroughPasses: report.ThroughPasses[0] + report.ThroughPasses[1],
                ThroughPassesCompleted: report.ThroughPassesCompleted[0] + report.ThroughPassesCompleted[1],
                Crosses: report.Crosses[0] + report.Crosses[1],
                CrossesVolleyed: report.CrossesVolleyed[0] + report.CrossesVolleyed[1],
                VolleyGoals: report.VolleyGoals[0] + report.VolleyGoals[1],
                ShotApertureSum: report.ShotApertureSum,
                LowApertureShots: report.LowApertureShots,
                LowApertureGoals: report.LowApertureGoals,
                BylineShots: report.BylineShots,
                ShotPosts: report.ShotPosts,
